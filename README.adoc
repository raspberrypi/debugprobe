...
[%header]
|===
| Probe |  2MHz |  4MHz |  6MHz |  8MHz

| picoprobe on Debug Probe HW
| 384s
| 380s
| 380s
| (-)

| YAPicoprobe on Pi Pico
| 237s
| 193s
| 178s
| 173s
...
That was tested with the following configuration:

```yaml
probes:
  <probe-unid>:
    frequency:       [2468]000000
    target_override: nrf52840
    test_binary:     l1_nrf52840-dk.bin
```

So `picoprobe` does have a problem.  Don't know if this is caused by the hardware
or by the firmware.  Will port yapicoprobe to the hardware soon.
