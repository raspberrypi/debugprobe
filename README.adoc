...
Another reason for this fork is that I wanted to play around with SWD, RTT etc pp, so
...
* sigrok probe - eight digital and three analog channels data collection
...
The probe also allows data collection for a https://sigrok.org/[sigrok] compatible
environment.  The module is based on work taken from https://github.com/pico-coder/sigrok-pico[sigrok-pico].
This also means, that https://sigrok.org/wiki/Libsigrok[libsigrok] has to be
...
* internal buffer of 100KByte which allows depending on 
  sample rate between 25000 and few hundred thousand samples
...
The plots above are taken with SWCLK = 15MHz.  Absolute time of the four commands
shrunk from ~25us to 18us.  Not bad.

But there are still gaps which may give more optimization opportunities.
